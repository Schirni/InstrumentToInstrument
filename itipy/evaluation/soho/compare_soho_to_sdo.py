...
# init
base_path = "/gpfs/gpfs0/robert.jarolim/itipy/soho_sdo_v6"
...
for soho_cube, iti_cube, sdo_cube in tqdm(zip(soho_maps, iti_maps, sdo_maps), total=len(selected_dates)):
    ...
    for i, (s_map, cmap, norm) in enumerate(zip(iti_cube, cmaps,
        ...
        data = s_map.data
        images['itipy'][aia_wavelengths[i]] = data
    ...
    for wl in aia_wavelengths:
        ...
        iti_img = images['itipy'][wl] / sdo_norms[wl].vmax
        ...
        evaluation['itipy'][wl]['psnr'].append(iti_psnr)
        evaluation['itipy'][wl]['cc'].append(iti_cc)
        evaluation['itipy'][wl]['ssim'].append(iti_ssim)

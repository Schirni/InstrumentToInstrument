...
# init
base_path = "/gpfs/gpfs0/robert.jarolim/itipy/soho_sdo_v4"
soho_data_path = '/gpfs/gpfs0/robert.jarolim/data/itipy/soho_iti2021_prep'
sdo_data_path = '/gpfs/gpfs0/robert.jarolim/data/itipy/sdo_comparison_iti2022'
...
evaluation = {'171': {'soho': {'ssim': [], 'psnr': [], 'cc': []}, 'itipy': {'ssim': [], 'psnr': [], 'cc': []}},
              '193': {'soho': {'ssim': [], 'psnr': [], 'cc': []}, 'itipy': {'ssim': [], 'psnr': [], 'cc': []}},
              '211': {'soho': {'ssim': [], 'psnr': [], 'cc': []}, 'itipy': {'ssim': [], 'psnr': [], 'cc': []}},
              '304': {'soho': {'ssim': [], 'psnr': [], 'cc': []}, 'itipy': {'ssim': [], 'psnr': [], 'cc': []}},
              'mag': {'soho': {'ssim': [], 'psnr': [], 'cc': []}, 'itipy': {'ssim': [], 'psnr': [], 'cc': []}}
              }
...
for soho_cube, iti_cube, sdo_cube in tqdm(zip(soho_maps, iti_maps, sdo_maps), total=len(selected_dates)):
    ...
    for i in range(4):
        ...
        evaluation[idx]['itipy']['ssim'].append(iti_ssim)
        evaluation[idx]['itipy']['psnr'].append(iti_psnr)
        evaluation[idx]['itipy']['cc'].append(iti_cc)
        ...
        print('ITI: SSIM %.03f; PSNR %.03f; CC %.03f;' %
              (np.mean(evaluation[idx]['itipy']['ssim']), np.mean(evaluation[idx]['itipy']['psnr']),
               np.mean(evaluation[idx]['itipy']['cc'])))
...
with open(os.path.join(prediction_path, 'evaluation.txt'), 'w') as f:
    ...
    for key, value in evaluation.items():
        print(key, file=f)
        print('SSIM %.03f; PSNR %.03f; CC %.03f;' % (
            np.mean(value['itipy']['ssim']), np.mean(value['itipy']['psnr']), np.mean(value['itipy']['cc']))
              , file=f)

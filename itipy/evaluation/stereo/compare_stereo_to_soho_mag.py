...
from itipy.data.editor import sdo_norms
from sunpy.map import Map, all_coordinates_from_map
from sunpy.visualization.colormaps import cm
from tqdm import tqdm

from itipy.translate import STEREOToSDOMagnetogram

from matplotlib import pyplot as plt

import numpy as np

base_path = "/gpfs/gpfs0/robert.jarolim/itipy/stereo_to_sdo_mag_v1"
prediction_path = os.path.join(base_path, 'evaluation')
os.makedirs(prediction_path, exist_ok=True)

soho_basenames = np.array([os.path.basename(f) for f in sorted(glob.glob('/gpfs/gpfs0/robert.jarolim/data/itipy/soho_iti2021_prep/171/*.fits'))])
soho_dates = np.array([parse(bn.split('.')[0]) for bn in soho_basenames])
stereo_basenames = np.array(
    [os.path.basename(f) for f in sorted(glob.glob('/gpfs/gpfs0/robert.jarolim/data/itipy/stereo_iti2021_prep/171/*.fits'))])
...
stereo_dataset = [[Map("/gpfs/gpfs0/robert.jarolim/data/itipy/stereo_iti2021_prep/%s/%s" % (wl, bn)) for wl in ['171', '195', '284', '304']] for bn in stereo_basenames]

soho_dataset = [[Map("/gpfs/gpfs0/robert.jarolim/data/itipy/soho_iti2021_prep/%s/%s" % (wl, bn)) for wl in ['171', '195', '284', '304', 'mag']] for bn in soho_basenames]
translator = STEREOToSDOMagnetogram(model_path=os.path.join(base_path, 'generator_AB.pt'))

result = translator.translate("/gpfs/gpfs0/robert.jarolim/data/itipy/stereo_iti2021_prep", basenames=stereo_basenames, return_arrays=True)
...
for date, (iti_maps, stereo_img, iti_img), soho_map_cube, stereo_map_cube in tqdm(zip(soho_dates, result, soho_dataset, stereo_dataset), total=len(stereo_basenames)):
    ...
    for c in range(5):
        s_map = iti_maps[c]
        if c == 4:
            axs[1, c].imshow(np.abs(s_map.data), cmap=cm.hmimag, vmin=-v_mag, vmax=v_mag)
        else:
    ...
    for c in range(5):
        ...
        if c == 4:
            ...
            axs[2, c].imshow(np.abs(s_map.data), cmap=cm.hmimag, vmin=-v_mag, vmax=v_mag)
        else:
    ...
    r = np.sqrt(hpc_coords.Tx ** 2 + hpc_coords.Ty ** 2) / s_map.rsun_obs
    data = s_map.data
    data = sdo_cmaps[-1](ImageNormalize(vmin=-v_mag, vmax=v_mag)(data))[..., :-1]
    data[r > 1] = [0, 0, 0]
    ...
    r = np.sqrt(hpc_coords.Tx ** 2 + hpc_coords.Ty ** 2) / s_map.rsun_obs
    data = s_map.data
    data = sdo_cmaps[-1](ImageNormalize(vmin=-v_mag, vmax=v_mag)(np.abs(data)))[..., :-1]
    data[r > 1] = [0,0,0]

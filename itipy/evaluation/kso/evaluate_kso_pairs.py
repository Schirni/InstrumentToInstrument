...
# create translator
translator = KSOLowToHigh(resolution=1024, model_path='/gpfs/gpfs0/robert.jarolim/itipy/kso_quality_v1/generator_AB.pt')
...
for (iti_map, kso_img, iti_img), hq_map, date in zip(translator.translate(lq_files), hq_maps, dates):
    ...
    result['itipy']['ssim'].append(iti_ssim)
    result['itipy']['psnr'].append(iti_psnr)
    result['itipy']['cc'].append(iti_cc)
    ...
    print('SSIM: KSO %.03f; ITI %.03f' % (np.mean(result['kso']['ssim']), np.mean(result['itipy']['ssim'])))
    print('PSNR: KSO %.03f; ITI %.03f' % (np.mean(result['kso']['psnr']), np.mean(result['itipy']['psnr'])))
    print('CC: KSO %.03f; ITI %.03f' % (np.mean(result['kso']['cc']), np.mean(result['itipy']['cc'])))
...
with open(os.path.join(prediction_path, 'evaluation.txt'), 'w') as f:
    ...
    print('ITI', file=f)
    print('SSIM %.03f; PSNR %.03f; CC %.03f;' % (
        np.mean(result['itipy']['ssim']), np.mean(result['itipy']['psnr']), np.mean(result['itipy']['cc']))
          , file=f)

...
for i, (hmi_map, hinode_map, hinode_path) in tqdm(enumerate(zip(hmi_maps, hinode_maps, hinode_paths)),
    ...
    # plt.imsave(p % (os.path.basename(hinode_path), 'itipy'), iti_data, cmap='gray')
    ...
    evaluation['itipy']['ssim'] += [ssim(iti_data, hinode_registered_iti)]
    evaluation['itipy']['psnr'] += [psnr(iti_data, hinode_registered_iti)]
    evaluation['itipy']['rmsc'] += [rms_contrast_diff(iti_data, hinode_registered_iti)]
    evaluation['itipy']['mae'] += [mae(iti_data, hinode_registered_iti)]
    evaluation['itipy']['cc'] += [image_correlation(iti_data, hinode_registered_iti)]
    ...
    [print("ITI", k, np.mean(v)) for k, v in evaluation['itipy'].items()]
...
with open(os.path.join(evaluation_path, 'evaluation.txt'), 'w') as f:
    ...
    print('ITI', file=f)
    print('(%f, %f, %f, %f, %f, %f)' % (
    np.mean(evaluation['itipy']['ssim']), np.mean(evaluation['itipy']['psnr']), np.mean(evaluation['itipy']['mae']), np.mean(evaluation['itipy']['rmsc']), fid_iti, np.mean(evaluation['itipy']['cc'])), file=f)
    print('HMI', file=f)
    print('(%f, %f, %f, %f, %f, %f)' % (

...
from itipy.data.dataset import StorageDataset, EUIDataset, SDODataset, EUIDataset2
...
parser = argparse.ArgumentParser(description='Train Solar Orbiter-To-SDO translations')
...
test_months = [11, 12]
train_months = list(range(2, 11))

sdo_dataset = SDODataset(sdo_path, wavelengths=[171, 304], resolution=1024, months=train_months)
sdo_dataset = StorageDataset(sdo_dataset,
                             sdo_converted_path,
                             ext_editors=[RandomPatchEditor((256, 256))])

fsi_dataset = EUIDataset2(fsi_path, months=train_months)
fsi_dataset = StorageDataset(fsi_dataset, fsi_converted_path,
                                ext_editors=[RandomPatchEditor((256, 256))])

sdo_valid = StorageDataset(SDODataset(sdo_path, wavelengths=[171, 304], resolution=1024, months=test_months, limit=100),
                           sdo_converted_path, ext_editors=[RandomPatchEditor((256, 256))])
fsi_valid = StorageDataset(EUIDataset2(fsi_path, months=test_months, limit=100),
                              fsi_converted_path, ext_editors=[RandomPatchEditor((256, 256))])
